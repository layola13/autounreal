...
FString NormalizeStandaloneAssetObjectPath_BP(const FString& AssetReference)
{
    FString Normalized = AssetReference;
...
FString BuildStandaloneAssetPackageDirName_BP(const FString& AssetObjectPath)
{
    FString Result = NormalizeStandaloneAssetObjectPath_BP(AssetObjectPath);
...
UObject* ResolveStandaloneAssetFromReference_BP(const FString& AssetReference, FString& OutAssetObjectPath)
{
    OutAssetObjectPath = NormalizeStandaloneAssetObjectPath_BP(AssetReference);
    if (OutAssetObjectPath.IsEmpty())
    {
        return nullptr;
    }

    if (UObject* Loaded = StaticLoadObject(UObject::StaticClass(), nullptr, *OutAssetObjectPath))
    {
        return Loaded;
    }

    const FString PackagePath = FPackageName::ObjectPathToPackageName(OutAssetObjectPath);
    if (!PackagePath.IsEmpty() && UEditorAssetLibrary::DoesAssetExist(PackagePath))
    {
        ...
        {
            OutAssetObjectPath = Loaded->GetPathName();
            return Loaded;
        }
    }
...
UObject* ResolveStandaloneAssetFromParams_BP(const TSharedPtr<FJsonObject>& Params, FString& OutAssetRef)
{
    OutAssetRef = ResolveBlueprintTargetParam(Params);
    if (OutAssetRef.IsEmpty())
    {
        OutAssetRef = GetFirstNonEmptyStringField_BP(
            Params,
            {TEXT("target"), TEXT("asset_path"), TEXT("path"), TEXT("soft_path_from_project_root")});
    }

    return ResolveStandaloneAssetFromReference_BP(OutAssetRef, OutAssetRef);
}

FString BuildDefaultStandaloneAssetBpyExportPath_BP(UObject* Asset)
{
    const FString AssetObjectPath = Asset ? Asset->GetPathName() : TEXT("/Game/Unknown.Unknown");
    return FPaths::Combine(
        FPaths::ProjectDir(),
        TEXT("ExportedBlueprints"),
        TEXT("bpy"),
        TEXT("other"),
        BuildStandaloneAssetPackageDirName_BP(AssetObjectPath),
        TEXT("__bp__.bp.py"));
}
...
FString BuildStandaloneAssetBpyText_BP(UObject* Asset)
{
    TSharedPtr<FJsonObject> Descriptor = MakeShared<FJsonObject>();
    Descriptor->SetStringField(TEXT("kind"), TEXT("standalone_asset"));
    Descriptor->SetStringField(TEXT("asset"), Asset ? Asset->GetPathName() : TEXT(""));
    Descriptor->SetStringField(TEXT("asset_class"), Asset ? Asset->GetClass()->GetPathName() : TEXT(""));
    Descriptor->SetStringField(TEXT("meta_file"), TEXT("asset_meta.py"));

    FString Output = TEXT("# Auto-generated by ExportBpy\n\n");
    Output += TEXT("bp = ");
    Output += SerializeJsonPretty_BP(Descriptor);
    Output += TEXT("\n");
    return Output;
}

bool IsStandaloneAssetBpyPackagePath_BP(const FString& InputPath)
{
    FString MainPath = InputPath;
    if (IFileManager::Get().DirectoryExists(*MainPath))
    {
        MainPath = FPaths::Combine(MainPath, TEXT("__bp__.bp.py"));
    }

    if (!IFileManager::Get().FileExists(*MainPath))
    {
        return false;
    }

    FString FileText;
    if (!FFileHelper::LoadFileToString(FileText, *MainPath))
    {
        return false;
    }

    return FileText.Contains(TEXT("\"kind\": \"standalone_asset\"")) ||
        FileText.Contains(TEXT("'kind': 'standalone_asset'")) ||
        FileText.Contains(TEXT("\"kind\":\"standalone_asset\""));
}
...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleGetBlueprintBpy(const TSharedPtr<FJsonObject>& Params)
{
    ...
    {
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(
            AssetRef.IsEmpty() ? TEXT("Missing asset target (target/asset_path/path)") : FString::Printf(TEXT("Asset not found: %s"), *AssetRef));
    }

    const FString BpyText = BuildStandaloneAssetBpyText_BP(Asset);
    TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
    ResultObj->SetBoolField(TEXT("success"), true);
    ResultObj->SetStringField(TEXT("asset_path"), Asset->GetPathName());
    ResultObj->SetStringField(TEXT("output_path"), BuildDefaultStandaloneAssetBpyExportPath_BP(Asset));
    ResultObj->SetStringField(TEXT("bpy_text"), BpyText);
    ResultObj->SetStringField(TEXT("python_text"), BpyText);
    ResultObj->SetStringField(TEXT("format"), TEXT("bpy_directory"));
    ResultObj->SetStringField(TEXT("producer"), TEXT("ExportBpy"));
...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleExportBlueprintBpy(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (OutputPath.IsEmpty())
    {
        OutputPath = BuildDefaultStandaloneAssetBpyExportPath_BP(Asset);
    }
    ...
    const FString ScriptId = FGuid::NewGuid().ToString(EGuidFormats::Digits);
    const FString CacheRoot = FPaths::Combine(BuildPythonExecutionCacheRoot_BP(), TEXT("ExportStandaloneAssetBpy"), ScriptId);
    IFileManager::Get().MakeDirectory(*CacheRoot, true);
    ...
    const FString AssetPathLiteral = NormalizePythonPathForScriptLiteral_BP(Asset->GetPathName());
    const FString OutputPathLiteral = NormalizePythonPathForScriptLiteral_BP(FPaths::ConvertRelativePathToFull(OutputPath));
    const FString ResultPath = FPaths::Combine(CacheRoot, TEXT("result.json"));
    const FString ResultPathLiteral = NormalizePythonPathForScriptLiteral_BP(FPaths::ConvertRelativePathToFull(ResultPath));
    const FString WrapperPath = FPaths::Combine(CacheRoot, TEXT("export_asset_bpy_wrapper.py"));
    ...
    const FString WrapperScript = FString::Printf(
        ...
        TEXT("plugin_py = r'''%s'''\n")
        TEXT("asset_path = r'''%s'''\n")
        TEXT("output_path = r'''%s'''\n")
        TEXT("result_path = r'''%s'''\n")
        TEXT("if plugin_py not in sys.path:\n")
        TEXT("    sys.path.insert(0, plugin_py)\n")
        TEXT("import asset_exporter\n")
        TEXT("asset_exporter = importlib.reload(asset_exporter)\n")
        TEXT("result = asset_exporter.export_asset_bpy_package(asset_path, output_path)\n")
        TEXT("with open(result_path, 'w', encoding='utf-8') as handle:\n")
        TEXT("    json.dump(result, handle, ensure_ascii=False, indent=2)\n")
        TEXT("if not result.get('success', False):\n")
        TEXT("    raise RuntimeError(str(result.get('error', 'standalone asset bpy export failed')))\n"),
        *PluginPythonDir,
        *AssetPathLiteral,
        *OutputPathLiteral,
        *ResultPathLiteral);
    ...
    {
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(
            FString::Printf(TEXT("Failed to write standalone asset bpy export wrapper script: %s"), *WrapperPath));
    }
    ...
    FString ExportResultJson;
    if (!FFileHelper::LoadFileToString(ExportResultJson, *ResultPath))
    {
        if (RunResult.IsValid())
        {
            return RunResult;
        }
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Failed to read standalone asset bpy export result"));
    }

    TSharedPtr<FJsonObject> ExportResult;
    if (!TryDeserializeJsonObject_BP(ExportResultJson, ExportResult) || !ExportResult.IsValid())
    {
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Failed to parse standalone asset bpy export result JSON"));
    }

    bool bSuccess = false;
    ExportResult->TryGetBoolField(TEXT("success"), bSuccess);
    if (!bSuccess)
    {
        FString ExportError;
        ExportResult->TryGetStringField(TEXT("error"), ExportError);
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(
            ExportError.IsEmpty() ? FString::Printf(TEXT("Failed to export standalone asset bpy package: %s"), *Asset->GetPathName()) : ExportError);
    }

    FString MainFilePath = BuildDefaultStandaloneAssetBpyExportPath_BP(Asset);
    ExportResult->TryGetStringField(TEXT("output_path"), MainFilePath);
...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleImportBlueprintPython(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (!InputPath.IsEmpty())
    {
        ...
        if (bIsBpyDslPath)
        {
            ...
            if (bIsStandaloneAssetPackage)
            {
                const FString NormalizedStandaloneTarget = NormalizeStandaloneAssetObjectPath_BP(TargetAssetRef);
                const FString ExistingPackagePath = FPackageName::ObjectPathToPackageName(NormalizedStandaloneTarget);
                if (!bOverwrite && !ExistingPackagePath.IsEmpty() && UEditorAssetLibrary::DoesAssetExist(ExistingPackagePath))
                {
                    return FEpicUnrealMCPCommonUtils::CreateErrorResponse(
                        FString::Printf(TEXT("Target asset already exists: %s"), *NormalizedStandaloneTarget));
                }
            }
            else
            {
                UBlueprint* ExistingBlueprint = FEpicUnrealMCPCommonUtils::FindBlueprint(TargetAssetRef);
            ...
            const FString WrapperScript = FString::Printf(
                ...
                TEXT("    'target_asset': imported_asset_path,\n")
                TEXT("    'import_mode': import_mode,\n")
                TEXT("    'compiled': bool(compiled),\n")
                TEXT("}\n")
                TEXT("with open(result_path, 'w', encoding='utf-8') as handle:\n")
                TEXT("    json.dump(result, handle, ensure_ascii=False, indent=2)\n")
                TEXT("if not ok:\n")
            ...
            ImportResult->TryGetBoolField(TEXT("compiled"), bCompiled);
            ResultObj->SetBoolField(TEXT("compiled"), bCompiled);
            FString ImportedAssetPath = TargetAssetRef;
            ImportResult->TryGetStringField(TEXT("target_asset"), ImportedAssetPath);
            if (!ImportedAssetPath.IsEmpty())
            {
                ResultObj->SetStringField(TEXT("target_asset"), ImportedAssetPath);
            }
            if (!bIsStandaloneAssetPackage)
            {
                ResultObj->SetStringField(TEXT("target_blueprint"), TargetAssetRef);
            }
            if (RunResult.IsValid())
            {
                ResultObj->SetObjectField(TEXT("python_run_result"), RunResult);
