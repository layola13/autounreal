...
namespace
{
FString NormalizeBlueprintReference(const FString& BlueprintReference)
{
...
FString GetBlueprintAssetName(const FString& BlueprintReference)
{
    FString AssetName = NormalizeBlueprintReference(BlueprintReference);

    const int32 DotIndex = AssetName.Find(TEXT("."), ESearchCase::CaseSensitive, ESearchDir::FromEnd);
    if (DotIndex != INDEX_NONE)
    {
        AssetName = AssetName.Mid(DotIndex + 1);
    }
    else if (AssetName.StartsWith(TEXT("/")))
    {
        AssetName = FPackageName::GetLongPackageAssetName(AssetName);
    }
    else if (AssetName.Contains(TEXT("/")))
    {
        AssetName = FPaths::GetCleanFilename(AssetName);
    }
...
bool TryLoadBlueprintFromReference(const FString& BlueprintReference, UBlueprint*& OutBlueprint)
{
    const FString NormalizedReference = NormalizeBlueprintReference(BlueprintReference);
    if (NormalizedReference.IsEmpty())
    {
        return false;
    }

    if (UBlueprint* ExistingBlueprint = FindObject<UBlueprint>(nullptr, *NormalizedReference))
    {
        OutBlueprint = ExistingBlueprint;
        return true;
    }

    if (UBlueprint* LoadedBlueprint = LoadObject<UBlueprint>(nullptr, *NormalizedReference))
    {
        OutBlueprint = LoadedBlueprint;
        return true;
    }

    FString AssetPath = NormalizedReference;
    ...
    if (AssetPath.StartsWith(TEXT("/")) && UEditorAssetLibrary::DoesAssetExist(AssetPath))
    {
        if (UBlueprint* EditorLoadedBlueprint = Cast<UBlueprint>(UEditorAssetLibrary::LoadAsset(AssetPath)))
        {
            OutBlueprint = EditorLoadedBlueprint;
            return true;
        }
    }
...
{
    const FString NormalizedBlueprintName = NormalizeBlueprintReference(BlueprintName);
    if (NormalizedBlueprintName.IsEmpty())
    {
        UE_LOG(LogTemp, Error, TEXT("FindBlueprintByName: Blueprint reference is empty"));
        return nullptr;
    }

    TArray<FString> CandidateReferences;
    CandidateReferences.AddUnique(NormalizedBlueprintName);

    if (!BlueprintName.IsEmpty())
    {
        CandidateReferences.AddUnique(BlueprintName);
    }

    if (!NormalizedBlueprintName.StartsWith(TEXT("/")))
    {
        CandidateReferences.AddUnique(FString::Printf(TEXT("/Game/Blueprints/%s.%s"), *NormalizedBlueprintName, *NormalizedBlueprintName));
        CandidateReferences.AddUnique(FString::Printf(TEXT("/Game/%s.%s"), *NormalizedBlueprintName, *NormalizedBlueprintName));
    }

    UBlueprint* Blueprint = nullptr;
    for (const FString& CandidateReference : CandidateReferences)
    {
        if (TryLoadBlueprintFromReference(CandidateReference, Blueprint))
        {
            return Blueprint;
        }
    }
    ...
    {
        UBlueprint* LoadedBlueprint = *It;
        if (!LoadedBlueprint)
        {
            continue;
        }

        if (LoadedBlueprint->GetName().Equals(TargetAssetName, ESearchCase::IgnoreCase) ||
            LoadedBlueprint->GetPathName().Contains(NormalizedBlueprintName))
        {
...
UClass* FEpicUnrealMCPCommonUtils::GetBlueprintGeneratedClassSafe(const UBlueprint* Blueprint)
{
    ...
    if (UClass* AuthoritativeClass = Blueprint->GeneratedClass->GetAuthoritativeClass())
    {
...
UClass* FEpicUnrealMCPCommonUtils::GetBlueprintCallableClassSafe(const UBlueprint* Blueprint)
{
    if (!Blueprint)
    {
        return nullptr;
    ...
    if (UClass* GeneratedClass = GetBlueprintGeneratedClassSafe(Blueprint))
    {
    ...
    {
        if (UClass* AuthoritativeClass = Blueprint->SkeletonGeneratedClass->GetAuthoritativeClass())
        {
            return AuthoritativeClass;
        }
...
TSharedPtr<FJsonObject> FEpicUnrealMCPCommonUtils::BlueprintClassInfoToJson(const UBlueprint* Blueprint)
{
    TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
    Result->SetBoolField(TEXT("has_blueprint"), Blueprint != nullptr);

    if (!Blueprint)
    {
        Result->SetStringField(TEXT("resolved_class_source"), TEXT("none"));
        return Result;
    }

    auto SetClassFields = [&Result](const FString& Prefix, const UClass* ClassValue)
    {
        Result->SetBoolField(Prefix + TEXT("_is_valid"), ClassValue != nullptr);
        Result->SetStringField(Prefix + TEXT("_name"), ClassValue ? ClassValue->GetName() : TEXT("None"));
        Result->SetStringField(Prefix + TEXT("_path"), ClassValue ? ClassValue->GetPathName() : TEXT(""));
    };

    const UClass* GeneratedClass = Blueprint->GeneratedClass;
    const UClass* AuthoritativeGeneratedClass = GeneratedClass ? GeneratedClass->GetAuthoritativeClass() : nullptr;
    const UClass* SkeletonClass = Blueprint->SkeletonGeneratedClass;
    const UClass* AuthoritativeSkeletonClass = SkeletonClass ? SkeletonClass->GetAuthoritativeClass() : nullptr;
    const UClass* ParentClass = Blueprint->ParentClass;
    const UClass* ResolvedCallableClass = GetBlueprintCallableClassSafe(Blueprint);
    const UClass* ResolvedGeneratedClass = GetBlueprintGeneratedClassSafe(Blueprint);
    ...
    SetClassFields(TEXT("parent_class"), ParentClass);
    SetClassFields(TEXT("generated_class"), GeneratedClass);
    SetClassFields(TEXT("authoritative_generated_class"), AuthoritativeGeneratedClass);
    SetClassFields(TEXT("skeleton_generated_class"), SkeletonClass);
    SetClassFields(TEXT("authoritative_skeleton_generated_class"), AuthoritativeSkeletonClass);
    SetClassFields(TEXT("resolved_generated_class"), ResolvedGeneratedClass);
    SetClassFields(TEXT("resolved_callable_class"), ResolvedCallableClass);

    FString ResolvedSource = TEXT("none");
    if (ResolvedCallableClass)
    {
        if ((AuthoritativeGeneratedClass && ResolvedCallableClass == AuthoritativeGeneratedClass) ||
            (GeneratedClass && ResolvedCallableClass == GeneratedClass))
        {
            ResolvedSource = TEXT("generated");
        }
        else if ((AuthoritativeSkeletonClass && ResolvedCallableClass == AuthoritativeSkeletonClass) ||
                 (SkeletonClass && ResolvedCallableClass == SkeletonClass))
        {
            ResolvedSource = TEXT("skeleton_generated");
        }
        else if (ParentClass && ResolvedCallableClass == ParentClass)
        {
            ResolvedSource = TEXT("parent_class");
        }
    }
    Result->SetStringField(TEXT("resolved_class_source"), ResolvedSource);
...
UK2Node_Event* FEpicUnrealMCPCommonUtils::CreateEventNode(UEdGraph* Graph, const FString& EventName, const FVector2D& Position)
{
    ...
    UClass* BlueprintClass = GetBlueprintCallableClassSafe(Blueprint);
    if (!BlueprintClass)
    {
        UE_LOG(LogTemp, Error, TEXT("CreateEventNode: Blueprint '%s' has no callable class"), *Blueprint->GetName());
        return nullptr;
    }
...
UK2Node_VariableGet* FEpicUnrealMCPCommonUtils::CreateVariableGetNode(UEdGraph* Graph, UBlueprint* Blueprint, const FString& VariableName, const FVector2D& Position)
{
    ...
    FProperty* Property = FindFProperty<FProperty>(BlueprintClass, VarName);
