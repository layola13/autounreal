...
{
    auto ResolveObjectFromPath = [](const FString& InTypeString) -> UObject*
    {
        ...
        UObject* LoadedObject = StaticLoadObject(UObject::StaticClass(), nullptr, *InTypeString);
        if (LoadedObject)
        {
            return LoadedObject;
        }

        if (InTypeString.StartsWith(TEXT("/")) && !InTypeString.Contains(TEXT(".")))
        {
            const FString AssetName = FPackageName::GetShortName(InTypeString);
            if (!AssetName.IsEmpty())
            {
                const FString ObjectPath = FString::Printf(TEXT("%s.%s"), *InTypeString, *AssetName);
        ...
        return FindObject<UObject>(nullptr, *InTypeString);
    };
    ...
    else if (TypeString.Equals(TEXT("AnimSequence"), ESearchCase::IgnoreCase))
    {
        PinType.PinCategory = UEdGraphSchema_K2::PC_Object;
        PinType.PinSubCategoryObject = UAnimSequence::StaticClass();
    }
    else if (TypeString.Equals(TEXT("AnimSequenceBase"), ESearchCase::IgnoreCase))
    {
        PinType.PinCategory = UEdGraphSchema_K2::PC_Object;
        PinType.PinSubCategoryObject = UAnimSequenceBase::StaticClass();
    }
    else if (TypeString.StartsWith(TEXT("/")))
    {
        UObject* LoadedObject = ResolveObjectFromPath(TypeString);
        if (UEnum* LoadedEnum = Cast<UEnum>(LoadedObject))
        {
            PinType.PinCategory = UEdGraphSchema_K2::PC_Byte;
            PinType.PinSubCategoryObject = LoadedEnum;
        }
        else if (UScriptStruct* LoadedStruct = Cast<UScriptStruct>(LoadedObject))
        {
            PinType.PinCategory = UEdGraphSchema_K2::PC_Struct;
        ...
        {
            PinType.PinCategory = UEdGraphSchema_K2::PC_Object;
            PinType.PinSubCategoryObject = LoadedClass;
        }
        else
        {
            // Default: float if resolution fails
            PinType.PinCategory = UEdGraphSchema_K2::PC_Real;
            PinType.PinSubCategory = UEdGraphSchema_K2::PC_Float;
        }
    }
