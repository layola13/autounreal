...
TSharedPtr<FJsonObject> FBlueprintNodeManager::AddNode(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		// Fallback: use top-level params for node configuration (pos_x, pos_y, etc.)
		NodeParams = Params;
	}
	...
	// Anim Graph Nodes (Pose Search / Motion Matching)
	else if (NodeType.Equals(TEXT("MotionMatching"), ESearchCase::IgnoreCase) ||
	         NodeType.Equals(TEXT("AnimGraphNode_MotionMatching"), ESearchCase::IgnoreCase))
	{
		NewNode = FAnimGraphNodeCreator::CreateMotionMatchingNode(Graph, NodeParams);
	}
	else if (NodeType.Equals(TEXT("PoseSearchHistoryCollector"), ESearchCase::IgnoreCase) ||
	         NodeType.Equals(TEXT("AnimGraphNode_PoseSearchHistoryCollector"), ESearchCase::IgnoreCase))
	{
		NewNode = FAnimGraphNodeCreator::CreatePoseSearchHistoryCollectorNode(Graph, NodeParams);
	}
	else if (NodeType.Equals(TEXT("PoseSearchComponentSpaceHistoryCollector"), ESearchCase::IgnoreCase) ||
	         NodeType.Equals(TEXT("AnimGraphNode_PoseSearchComponentSpaceHistoryCollector"), ESearchCase::IgnoreCase))
	{
		NewNode = FAnimGraphNodeCreator::CreatePoseSearchComponentSpaceHistoryCollectorNode(Graph, NodeParams);
	}
	// Specialized Nodes
	...
	else if (NodeType.Equals(TEXT("Knot"), ESearchCase::IgnoreCase))
	{
		NewNode = FSpecializedNodeCreator::CreateKnotNode(Graph, NodeParams);
	         ...
	         NodeType.Equals(TEXT("InputAction"), ESearchCase::IgnoreCase))
	{
		NewNode = FSpecializedNodeCreator::CreateEnhancedInputActionNode(Graph, NodeParams);
	}
	...
	TArray<TSharedPtr<FJsonValue>> AppliedPinDefaults;
	if (UK2Node* NewK2Node = Cast<UK2Node>(NewNode))
	{
		if (FNodePropertyManager::ExtractPinDefaultSpecs(NodeParams, PinDefaults) && PinDefaults.Num() > 0)
		{
			FString PinDefaultError;
			if (!FNodePropertyManager::ApplyPinDefaults(BP, Graph, NewK2Node, PinDefaults, AppliedPinDefaults, PinDefaultError))
			{
				FBlueprintEditorUtils::RemoveNode(BP, NewNode, true);
				Graph->NotifyGraphChanged();
				FBlueprintEditorUtils::MarkBlueprintAsModified(BP);
				return CreateErrorResponse(FString::Printf(TEXT("Failed to apply pin defaults for new node: %s"), *PinDefaultError));
			}
		}
	}
	else if (FNodePropertyManager::ExtractPinDefaultSpecs(NodeParams, PinDefaults) && PinDefaults.Num() > 0)
	{
		FBlueprintEditorUtils::RemoveNode(BP, NewNode, true);
		Graph->NotifyGraphChanged();
		FBlueprintEditorUtils::MarkBlueprintAsModified(BP);
		return CreateErrorResponse(TEXT("pin_defaults are only supported for K2 nodes"));
	}

	// Notify changes
	if (AppliedPinDefaults.Num() == 0)
	{
		Graph->NotifyGraphChanged();
		FBlueprintEditorUtils::MarkBlueprintAsModified(BP);
	}
	...
	TSharedPtr<FJsonObject> Response = CreateSuccessResponse(NewNode, NodeType);
	if (AppliedPinDefaults.Num() > 0)
	{
		Response->SetArrayField(TEXT("pin_defaults_applied"), AppliedPinDefaults);
		Response->SetNumberField(TEXT("pin_defaults_applied_count"), AppliedPinDefaults.Num());
	}
...
TSharedPtr<FJsonObject> FBlueprintNodeManager::CreateSuccessResponse(const UEdGraphNode* Node, const FString& NodeType)
{
