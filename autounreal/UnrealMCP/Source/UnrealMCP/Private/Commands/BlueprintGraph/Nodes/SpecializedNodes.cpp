...
UK2Node* FSpecializedNodeCreator::CreateEnhancedInputActionNode(UEdGraph* Graph, const TSharedPtr<FJsonObject>& Params)
{
	if (!Graph || !Params.IsValid())
	{
		return nullptr;
	...
	else if (Params->TryGetStringField(TEXT("target_function"), ActionPath))
	{
		// Fallback: tool schema can only pass target_function
		if (ActionPath.StartsWith(TEXT("/")))
		{
			InputAction = Cast<UInputAction>(UEditorAssetLibrary::LoadAsset(ActionPath));
		}
		else
	...
	else if (Params->TryGetStringField(TEXT("variable_name"), ActionPath))
	{
		if (ActionPath.StartsWith(TEXT("/")))
		{
			InputAction = Cast<UInputAction>(UEditorAssetLibrary::LoadAsset(ActionPath));
	...
	else if (Params->TryGetStringField(TEXT("message"), ActionPath))
	{
		if (ActionPath.StartsWith(TEXT("/")))
		{
			InputAction = Cast<UInputAction>(UEditorAssetLibrary::LoadAsset(ActionPath));
	...
	else if (Params->TryGetStringField(TEXT("input_action"), ActionName))
	{
		// Try to find by asset name via AssetRegistry
	...
	// If we got a name fallback, resolve it now
	if (!InputAction && !ActionName.IsEmpty())
	{
		FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry"));
		TArray<FAssetData> Assets;
		AssetRegistryModule.Get().GetAssetsByClass(UInputAction::StaticClass()->GetClassPathName(), Assets, true);
	...
	Graph->AddNode(ActionNode, true, false);
	FNodeCreatorUtils::InitializeK2Node(ActionNode, Graph);
