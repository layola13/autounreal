...
from helpers.blueprint_graph import graph_formatter
...
class UnrealConnection:
    ...
    LARGE_OPERATION_COMMANDS = {
        ...
        "get_asset_structs",
        "review_blueprint",
        "create_and_validate_blueprint_plan",
        "export_blueprint",
        "exportblueprint",
        "export_blueprint_functions",
        "split_blueprint_functions",
        "import_blueprint_functions",
        "reimport_blueprint_functions",
        "compile_live_coding",
        "compile_cpp",
        "compile_unreal_code",
        "trigger_live_coding"
...
def _maybe_add_param(params: Dict[str, Any], key: str, value: Any) -> None:
    """Add a parameter when it has a meaningful value."""
    if value is None:
        return
...
def _apply_blueprint_target_params(
    params: Dict[str, Any],
    target: str = "",
    asset_path: str = "",
    blueprint_path: str = "",
    ...
    """Apply the common blueprint target aliases accepted by the Unreal bridge."""
    _maybe_add_param(params, "target", target)
    _maybe_add_param(params, "asset_path", asset_path)
    _maybe_add_param(params, "blueprint_path", blueprint_path)
    _maybe_add_param(params, "soft_path_from_project_root", soft_path_from_project_root)
    _maybe_add_param(params, "blueprint_name", blueprint_name)
    _maybe_add_param(params, "blueprint", blueprint)
    return params


def _forward_unreal_command(command: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Forward a command to the Unreal bridge and normalize common error handling."""
...
def save_project() -> Dict[str, Any]:
    """Save dirty project packages in the editor."""
    return _forward_unreal_command("save_project")


@mcp.tool()
def save_all() -> Dict[str, Any]:
    """Alias for saving all dirty project packages in the editor."""
    return _forward_unreal_command("save_all")
...
def copy_value(
    ...
    """
    Copy one property value from a source Blueprint endpoint to a target endpoint.

    Endpoint object format:
      {
        "blueprint_name": "CBP_Source",
        "component_name": "Mesh",          # optional, omit for CDO
        "property_name": "skeletal_mesh_asset"
      }

    Args:
        source: Source endpoint object
        target: Target endpoint object
        compile_blueprint: Compile target Blueprint after copy
    """
    unreal = get_unreal_connection()
    if not unreal:
        return {"success": False, "message": "Failed to connect to Unreal Engine"}
    ...
    try:
        ...
        response = unreal.send_command("copy_value", params)
        return response or {"success": False, "message": "No response from Unreal"}
    except Exception as e:
        logger.error(f"copy_value error: {e}")
        return {"success": False, "message": str(e)}
...
def find_and_replace(
    ...
    """
    Find and replace Blueprint graph references from old_ref to new_ref.

    This updates node member references, dynamic cast targets, and pin type references.

    Args:
        source: Blueprint name or path
        old_ref: Old class/object reference path
        new_ref: New class/object reference path
        compile_blueprint: Compile Blueprint after replacement
        graph_name: Optional graph name filter (empty means all graphs)
    """
    unreal = get_unreal_connection()
    if not unreal:
        return {"success": False, "message": "Failed to connect to Unreal Engine"}
    ...
    try:
        ...
        response = unreal.send_command("find_and_replace", params)
        return response or {"success": False, "message": "No response from Unreal"}
    except Exception as e:
        logger.error(f"find_and_replace error: {e}")
        return {"success": False, "message": str(e)}

@mcp.tool()
def run_python(code: str = "", file_path: str = "") -> Dict[str, Any]:
    """
    Execute Python in the Unreal Editor.

    Args:
        code: Python code to execute
        file_path: Optional path to a .py file to execute
    """
    unreal = get_unreal_connection()
    if not unreal:
        return {"success": False, "message": "Failed to connect to Unreal Engine"}
    ...
    if file_path and not code:
        # Use exec(open(...).read()) to run a script file
        code = f"exec(open(r'{file_path}').read())"
    ...
    try:
        params = {"code": code}
        response = unreal.send_command("run_python", params)
        return response or {"success": False, "message": "No response from Unreal"}
    except Exception as e:
...
@mcp.tool()
def fetch_blueprint_best_practices() -> Dict[str, Any]:
    """Fetch Blueprint editing best practices from the Unreal bridge."""
    return _forward_unreal_command("fetch_blueprint_best_practices")
...
def bp_agent(
    ...
    """High-level Blueprint helper exposed by the Unreal bridge."""
    params = {"request_text": request_text}
    _maybe_add_param(params, "relevant_data", relevant_data)
    _maybe_add_param(params, "previous_context", previous_context)
    _maybe_add_param(params, "short_description", short_description)
    return _forward_unreal_command("bp_agent", params)


@mcp.tool()
def add_blueprint_node_to_strand(
    soft_path_from_project_root: str,
    strand_name: str,
    nodes: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """Add nodes to a Blueprint strand."""
    return _forward_unreal_command(
        "add_blueprint_node_to_strand",
        {
...
def connect_blueprint_nodes(
    ...
    """Connect Blueprint nodes in a strand."""
    return _forward_unreal_command(
...
def disconnect_blueprint_nodes(
    ...
    """Disconnect Blueprint nodes in a strand."""
    return _forward_unreal_command(
...
def remove_blueprint_nodes(
    ...
    strand_name: str,
    graph_type: str,
    node_unique_names: List[str]
) -> Dict[str, Any]:
    """Remove Blueprint nodes from a graph."""
    return _forward_unreal_command(
        "remove_blueprint_nodes",
        {
            ...
            "node_unique_names": node_unique_names
        }
    )


@mcp.tool()
def set_node_pins_defaults(
    soft_path_from_project_root: str,
    strand_name: str,
    nodes: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """Set default values on Blueprint node pins."""
    return _forward_unreal_command(
        "set_node_pins_defaults",
        {
            "soft_path_from_project_root": soft_path_from_project_root,
            "strand_name": strand_name,
            "nodes": nodes
...
def create_blueprint_interface(
    soft_path_from_project_root: str,
    interface_functions: Any = None
) -> Dict[str, Any]:
    """Create a Blueprint Interface asset."""
    params: Dict[str, Any] = {"soft_path_from_project_root": soft_path_from_project_root}
    _maybe_add_param(params, "interface_functions", interface_functions)
    return _forward_unreal_command("create_blueprint_interface", params)
...
def edit_blueprint(
    ...
    properties_settings: Optional[Dict[str, Any]] = None,
    functions: Optional[List[Dict[str, Any]]] = None,
    components: Optional[List[Dict[str, Any]]] = None,
    interfaces: Optional[List[str]] = None
) -> Dict[str, Any]:
    """Edit a Blueprint with structured properties, functions, components, and interfaces."""
    params: Dict[str, Any] = {}
    ...
    _maybe_add_param(params, "properties", properties)
    _maybe_add_param(params, "properties_settings", properties_settings)
    _maybe_add_param(params, "functions", functions)
    _maybe_add_param(params, "components", components)
    _maybe_add_param(params, "interfaces", interfaces)
    return _forward_unreal_command("edit_blueprint", params)
...
def set_skeletal_mesh_properties(
    ...
    _maybe_add_param(params, "skeletal_mesh", skeletal_mesh)
    return _forward_unreal_command("set_skeletal_mesh_properties", params)


@mcp.tool()
def spawn_blueprint_actors(actors: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Spawn actors from Blueprint assets in the current level."""
    return _forward_unreal_command("spawn_blueprint_actors", {"actors": actors})
...
def spawn_blueprint_actor(
    ...
    """Spawn a single actor from a Blueprint asset in the current level."""
    params: Dict[str, Any] = {
        "blueprint_name": blueprint_name,
        "actor_name": actor_name
    }
    _maybe_add_param(params, "location", location)
    _maybe_add_param(params, "rotation", rotation)
    return _forward_unreal_command("spawn_blueprint_actor", params)
...
def execute_unreal_python(
    ...
    _maybe_add_param(params, "request_text", request_text)
    _maybe_add_param(params, "short_description", short_description)
    return _forward_unreal_command("execute_unreal_python", params)


@mcp.tool()
def create_gameplay_tag(tag_names: List[str], ini_file: str = "") -> Dict[str, Any]:
    """Create gameplay tags."""
    params: Dict[str, Any] = {"tag_names": tag_names}
    _maybe_add_param(params, "ini_file", ini_file)
    return _forward_unreal_command("create_gameplay_tag", params)


@mcp.tool()
def create_input_actions(input_actions: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Create Enhanced Input action assets."""
    return _forward_unreal_command("create_input_actions", {"input_actions": input_actions})


@mcp.tool()
def add_input_action_to_mapping_context(
    ...
    """Add an input action binding to a mapping context."""
    params: Dict[str, Any] = {
        "mapping_context_path": mapping_context_path,
        "input_action_path": input_action_path,
        "key": key
    }
    _maybe_add_param(params, "negate", negate)
    _maybe_add_param(params, "scale", scale)
    _maybe_add_param(params, "swizzle_order", swizzle_order)
    return _forward_unreal_command("add_input_action_to_mapping_context", params)


@mcp.tool()
def add_or_replace_rows_in_data_table(soft_path: str, row_datas: Dict[str, Any]) -> Dict[str, Any]:
    """Add or replace rows in a DataTable."""
    return _forward_unreal_command(
        "add_or_replace_rows_in_data_table",
        {"soft_path": soft_path, "row_datas": row_datas}
    )


@mcp.tool()
def remove_rows_from_data_table(soft_path: str, row_names: List[str]) -> Dict[str, Any]:
    """Remove rows from a DataTable."""
    return _forward_unreal_command(
        "remove_rows_from_data_table",
        {"soft_path": soft_path, "row_names": row_names}
    )


@mcp.tool()
def edit_enumeration(soft_path: str, enum_fields: List[str]) -> Dict[str, Any]:
    """Edit a Blueprint enumeration asset."""
    return _forward_unreal_command("edit_enumeration", {"soft_path": soft_path, "enum_fields": enum_fields})


@mcp.tool()
def edit_structure(
    soft_path: str,
    default_structure_data: Dict[str, Any],
    type_hints: Dict[str, Any]
) -> Dict[str, Any]:
    """Edit a Blueprint structure asset."""
    return _forward_unreal_command(
        "edit_structure",
        {
            "soft_path": soft_path,
            "default_structure_data": default_structure_data,
...
@mcp.tool()
def get_data_asset_types(dummy_input: str = "") -> Dict[str, Any]:
    """Get available data asset types."""
    return _forward_unreal_command("get_data_asset_types", {"dummy_input": dummy_input})


@mcp.tool()
def get_data_asset_type_info(data_asset_type: str) -> Dict[str, Any]:
    """Get information about a specific data asset type."""
    return _forward_unreal_command("get_data_asset_type_info", {"data_asset_type": data_asset_type})


@mcp.tool()
def create_text_file(files: List[Dict[str, Any]], explanation: str = "") -> Dict[str, Any]:
    """Create new text files inside allowed project paths."""
    params: Dict[str, Any] = {"files": files}
    _maybe_add_param(params, "explanation", explanation)
    return _forward_unreal_command("create_text_file", params)


@mcp.tool()
def edit_text_file(path: str, edits: List[Dict[str, Any]], explanation: str = "") -> Dict[str, Any]:
    """Edit an existing text file inside allowed project paths."""
    params: Dict[str, Any] = {
        "path": path,
        "edits": edits
    }
    _maybe_add_param(params, "explanation", explanation)
    return _forward_unreal_command("edit_text_file", params)


@mcp.tool()
def create_new_todo_list(todo_list_data: List[str]) -> Dict[str, Any]:
    """Create or replace the compact todo list."""
    return _forward_unreal_command("create_new_todo_list", {"todo_list_data": todo_list_data})


@mcp.tool()
def get_todo_list() -> Dict[str, Any]:
    """Get the current todo list."""
    return _forward_unreal_command("get_todo_list")


@mcp.tool()
def edit_todo_list(codes: str) -> Dict[str, Any]:
    """Edit todo items using compact priority/status codes."""
    return _forward_unreal_command("edit_todo_list", {"codes": codes})


@mcp.tool()
def get_headless_status() -> Dict[str, Any]:
    """Get headless Unreal status."""
    return _forward_unreal_command("get_headless_status")


@mcp.tool()
def shutdown_headless() -> Dict[str, Any]:
    """Shut down the managed headless Unreal instance when one exists."""
    return _forward_unreal_command("shutdown_headless")
...
@mcp.tool()
def get_unreal_context(only_get_drag_dropped_objects: Optional[bool] = None) -> Dict[str, Any]:
    """Get current Unreal editor context."""
    params: Dict[str, Any] = {}
    _maybe_add_param(params, "only_get_drag_dropped_objects", only_get_drag_dropped_objects)
    return _forward_unreal_command("get_unreal_context", params)
...
@mcp.tool()
def fetch_animation_skill(skill_name: str) -> Dict[str, Any]:
    """Fetch the local animation skill reference used by UnrealMCP."""
    return _forward_unreal_command("fetch_animation_skill", {"skill_name": skill_name})


@mcp.tool()
def create_or_edit_plan(markdown_text: str, plan_name: str, title: str = "") -> Dict[str, Any]:
    """Create a versioned plan markdown file under Saved/.Aura/plans."""
    params: Dict[str, Any] = {
        "markdown_text": markdown_text,
        "plan_name": plan_name
    }
    _maybe_add_param(params, "title", title)
    return _forward_unreal_command("create_or_edit_plan", params)


@mcp.tool()
def fetch_gas_best_practices() -> Dict[str, Any]:
    """Fetch local Gameplay Ability System best practices."""
    return _forward_unreal_command("fetch_gas_best_practices")


@mcp.tool()
def fetch_ui_best_practices() -> Dict[str, Any]:
    """Fetch local UMG/UI best practices."""
    return _forward_unreal_command("fetch_ui_best_practices")


@mcp.tool()
def import_AssetCreation_understanding() -> Dict[str, Any]:
    """Import local Unreal asset creation understanding notes."""
    return _forward_unreal_command("import_AssetCreation_understanding")


@mcp.tool()
def import_AssetRegistry_understanding() -> Dict[str, Any]:
    """Import local Unreal asset registry understanding notes."""
    return _forward_unreal_command("import_AssetRegistry_understanding")


@mcp.tool()
def import_AssetType_Level_understanding() -> Dict[str, Any]:
    """Import local Unreal level asset understanding notes."""
    return _forward_unreal_command("import_AssetType_Level_understanding")


@mcp.tool()
def import_AssetValidation_understanding() -> Dict[str, Any]:
    """Import local Unreal asset validation understanding notes."""
    return _forward_unreal_command("import_AssetValidation_understanding")


@mcp.tool()
def import_Color_understanding() -> Dict[str, Any]:
    """Import local Unreal color handling understanding notes."""
    return _forward_unreal_command("import_Color_understanding")


@mcp.tool()
def import_FileSystem_understanding() -> Dict[str, Any]:
    """Import local Unreal file system understanding notes."""
    return _forward_unreal_command("import_FileSystem_understanding")


@mcp.tool()
def import_Logs_understanding() -> Dict[str, Any]:
    """Import local Unreal logging understanding notes."""
    return _forward_unreal_command("import_Logs_understanding")


@mcp.tool()
def import_Subsystems_understanding() -> Dict[str, Any]:
    """Import local Unreal subsystem understanding notes."""
    return _forward_unreal_command("import_Subsystems_understanding")


@mcp.tool()
def inspect_current_level(python_script: str, query_text: str) -> Dict[str, Any]:
    """Inspect the current level through the Unreal bridge's safe inspector path."""
    return _forward_unreal_command(
        "inspect_current_level",
        {
...
@mcp.tool()
def query_unreal_project_assets(query: str, num_files: Optional[int] = None) -> Dict[str, Any]:
    """Query Unreal project assets by keyword."""
    params: Dict[str, Any] = {"query": query}
    _maybe_add_param(params, "num_files", num_files)
    return _forward_unreal_command("query_unreal_project_assets", params)


@mcp.tool()
def quicksearch(query: str, max_results: Optional[int] = None) -> Dict[str, Any]:
    """Perform the quicksearch bridge query."""
    params: Dict[str, Any] = {"query": query}
    _maybe_add_param(params, "max_results", max_results)
    return _forward_unreal_command("quicksearch", params)
...
def grep(
    ...
    head_limit: str = "",
    search_filenames: Optional[bool] = None,
    search_engine: Optional[bool] = None
) -> Dict[str, Any]:
    """Search code files with the Unreal bridge ripgrep wrapper."""
    params: Dict[str, Any] = {"pattern": pattern}
    _maybe_add_param(params, "path", path)
    _maybe_add_param(params, "type", type)
    _maybe_add_param(params, "glob", glob)
    _maybe_add_param(params, "output_mode", output_mode)
    _maybe_add_param(params, "-A", A)
    _maybe_add_param(params, "-B", B)
    _maybe_add_param(params, "-C", C)
    _maybe_add_param(params, "i", i)
    _maybe_add_param(params, "multiline", multiline)
    _maybe_add_param(params, "head_limit", head_limit)
    _maybe_add_param(params, "search_filenames", search_filenames)
    _maybe_add_param(params, "search_engine", search_engine)
    return _forward_unreal_command("grep", params)
...
def get_code_examples(
    ...
    include_engine_source: Optional[bool] = None
) -> Dict[str, Any]:
    """Search Unreal code examples."""
    params: Dict[str, Any] = {"keywords": keywords}
    _maybe_add_param(params, "time_limit", time_limit)
    _maybe_add_param(params, "include_engine_source", include_engine_source)
    return _forward_unreal_command("get_code_examples", params)


@mcp.tool()
def get_asset_meta(asset_path: str, parts: List[str]) -> Dict[str, Any]:
    """Get metadata for an asset."""
    return _forward_unreal_command("get_asset_meta", {"asset_path": asset_path, "parts": parts})


@mcp.tool()
def get_asset_graph(asset_path: str, strand_names: List[str]) -> Dict[str, Any]:
    """Get graph data for an asset."""
    return _forward_unreal_command("get_asset_graph", {"asset_path": asset_path, "strand_names": strand_names})


@mcp.tool()
def get_asset_structs(asset_path: str) -> Dict[str, Any]:
    """Get additional struct metadata referenced by an asset."""
    return _forward_unreal_command("get_asset_structs", {"asset_path": asset_path})


@mcp.tool()
def get_blueprint_material_properties(soft_path_from_project_root: str) -> Dict[str, Any]:
    """Get material properties referenced by a Blueprint."""
    return _forward_unreal_command(
        "get_blueprint_material_properties",
        {"soft_path_from_project_root": soft_path_from_project_root}
    )
...
def review_blueprint(
    asset_path: str,
    strand_names: Optional[List[str]] = None,
    previous_asset_path: str = "",
    custom_instructions: str = ""
) -> Dict[str, Any]:
    """Run the Blueprint review helper."""
    params: Dict[str, Any] = {"asset_path": asset_path}
    _maybe_add_param(params, "strand_names", strand_names)
    _maybe_add_param(params, "previous_asset_path", previous_asset_path)
    _maybe_add_param(params, "custom_instructions", custom_instructions)
    return _forward_unreal_command("review_blueprint", params)


@mcp.tool()
def create_and_validate_blueprint_plan(
    blueprint_name: str,
    soft_path_from_project_root: str,
    user_prompt: str,
    graph_page: str = "",
    graph_type: str = "",
    only_change_part_of_graph: Optional[bool] = None,
    specific_properties_to_change: Optional[List[str]] = None,
    additional_blueprint_paths: Optional[List[str]] = None
) -> Dict[str, Any]:
    """Create and validate a Blueprint implementation plan."""
    params: Dict[str, Any] = {
        "blueprint_name": blueprint_name,
        "soft_path_from_project_root": soft_path_from_project_root,
        "user_prompt": user_prompt
    }
    _maybe_add_param(params, "graph_page", graph_page)
    _maybe_add_param(params, "graph_type", graph_type)
    _maybe_add_param(params, "only_change_part_of_graph", only_change_part_of_graph)
    _maybe_add_param(params, "specific_properties_to_change", specific_properties_to_change)
    _maybe_add_param(params, "additional_blueprint_paths", additional_blueprint_paths)
    return _forward_unreal_command("create_and_validate_blueprint_plan", params)
...
def get_text_file_contents(
    ...
    """Read text file contents via the Unreal inspector bridge."""
    params: Dict[str, Any] = {"full_file_path": full_file_path}
    _maybe_add_param(params, "line_start", line_start)
    _maybe_add_param(params, "num_lines", num_lines)
    return _forward_unreal_command("get_text_file_contents", params)


@mcp.tool()
def get_available_actors_in_level(
    level: str = "",
    get_flags: str = "",
    filter_actor_class: str = "",
    filter_actor_label: str = ""
) -> Dict[str, Any]:
    """List available actors in a level with optional filters."""
    return _forward_unreal_command(
        "get_available_actors_in_level",
        {
            "level": level,
            "get_flags": get_flags,
            "filter_actor_class": filter_actor_class,
            "filter_actor_label": filter_actor_label
        }
    )


@mcp.tool()
def get_enums(query: str = "", max_results: Optional[int] = None) -> Dict[str, Any]:
    """Get enums from the project."""
    params: Dict[str, Any] = {"query": query}
    _maybe_add_param(params, "max_results", max_results)
    return _forward_unreal_command("get_enums", params)


@mcp.tool()
def get_gameplay_tags(query: str = "", max_results: Optional[int] = None) -> Dict[str, Any]:
    """Get gameplay tags from the project."""
    params: Dict[str, Any] = {"query": query}
    _maybe_add_param(params, "max_results", max_results)
    return _forward_unreal_command("get_gameplay_tags", params)


@mcp.tool()
def read_datatable_keys(soft_path: str) -> Dict[str, Any]:
    """Read DataTable row keys."""
    return _forward_unreal_command("read_datatable_keys", {"soft_path": soft_path})


@mcp.tool()
def read_datatable_values(soft_path: str, row_names: List[str]) -> Dict[str, Any]:
    """Read DataTable row values."""
    return _forward_unreal_command("read_datatable_values", {"soft_path": soft_path, "row_names": row_names})
...
def get_blueprint_properties(
    ...
    """Get the properties of a Blueprint file."""
    params: Dict[str, Any] = {}
    _apply_blueprint_target_params(
        params,
        target=target,
    ...
    return _forward_unreal_command("get_blueprint_properties", params)
...
def get_blueprint_component_properties(
    ...
    """Get the properties of a component in a Blueprint file."""
    params: Dict[str, Any] = {
        "component_name": component_name,
        "component_type": component_type
    }
    ...
    return _forward_unreal_command("get_blueprint_component_properties", params)


@mcp.tool()
def get_blueprint_properties_specifiers(
    ...
    """Get specifiers for Blueprint properties."""
    params: Dict[str, Any] = {"property_names": property_names}
    _apply_blueprint_target_params(
        params,
        target=target,
        asset_path=asset_path,
    ...
    return _forward_unreal_command("get_blueprint_properties_specifiers", params)
...
def get_blueprint_class_info(
    ...
    """Get high-level class information for a Blueprint."""
    params: Dict[str, Any] = {}
    _apply_blueprint_target_params(
        params,
        target=target,
...
def get_blueprint_meta(
    ...
    property_value_depth: Optional[int] = None,
    property_value_max_items: Optional[int] = None
) -> Dict[str, Any]:
    """Get the Blueprint .meta text content."""
    params: Dict[str, Any] = {}
    _apply_blueprint_target_params(
        params,
        target=target,
    ...
    _maybe_add_param(params, "parts", parts)
    _maybe_add_param(params, "property_value_depth", property_value_depth)
    _maybe_add_param(params, "property_value_max_items", property_value_max_items)
    return _forward_unreal_command("get_blueprint_meta", params)


@mcp.tool()
def export_blueprint_meta(
    target: str = "",
    asset_path: str = "",
    blueprint_path: str = "",
    soft_path_from_project_root: str = "",
    ...
    """Export Blueprint .meta text to disk."""
    params: Dict[str, Any] = {}
    _apply_blueprint_target_params(
        params,
        target=target,
    ...
    _maybe_add_param(params, "directory", directory)
    _maybe_add_param(params, "parts", parts)
    _maybe_add_param(params, "property_value_depth", property_value_depth)
    _maybe_add_param(params, "property_value_max_items", property_value_max_items)
    return _forward_unreal_command("export_blueprint_meta", params)


@mcp.tool()
def export_asset_meta(
    target: str = "",
    asset_path: str = "",
    blueprint_path: str = "",
    soft_path_from_project_root: str = "",
    ...
    """Export Blueprint .meta text using the asset alias command."""
    params: Dict[str, Any] = {}
    _apply_blueprint_target_params(
        params,
        target=target,
    ...
    _maybe_add_param(params, "property_value_depth", property_value_depth)
    _maybe_add_param(params, "property_value_max_items", property_value_max_items)
    return _forward_unreal_command("export_asset_meta", params)
...
def get_asset_py(
    ...
    """Get the Blueprint Python export text using the asset alias."""
    params: Dict[str, Any] = {}
    _apply_blueprint_target_params(
        params,
        target=target,
    ...
    return _forward_unreal_command("get_asset_py", params)


@mcp.tool()
def get_blueprint_py(
    target: str = "",
    asset_path: str = "",
    blueprint_path: str = "",
    soft_path_from_project_root: str = "",
    ...
    """Get the Blueprint Python export text."""
    params: Dict[str, Any] = {}
    _apply_blueprint_target_params(
        params,
        target=target,
    ...
    return _forward_unreal_command("get_blueprint_py", params)
...
def export_asset_py(
    ...
    """Export the Blueprint Python file using the asset alias."""
    params: Dict[str, Any] = {}
    _apply_blueprint_target_params(
        params,
        target=target,
    ...
    _maybe_add_param(params, "output_file", output_file)
    _maybe_add_param(params, "path", path)
    return _forward_unreal_command("export_asset_py", params)


@mcp.tool()
def export_blueprint_py(
    target: str = "",
    asset_path: str = "",
    blueprint_path: str = "",
    soft_path_from_project_root: str = "",
    ...
    """Export the Blueprint Python file."""
    params: Dict[str, Any] = {}
    _apply_blueprint_target_params(
        params,
        target=target,
    ...
    _maybe_add_param(params, "output_file", output_file)
    _maybe_add_param(params, "path", path)
    return _forward_unreal_command("export_blueprint_py", params)
...
def get_asset_bpy(
    ...
    """Get ExportBpy single-file Blueprint bpy text using the asset alias."""
    params: Dict[str, Any] = {}
    _apply_blueprint_target_params(
        params,
        target=target,
    ...
    return _forward_unreal_command("get_asset_bpy", params)


@mcp.tool()
def get_blueprint_bpy(
    target: str = "",
    asset_path: str = "",
    blueprint_path: str = "",
    soft_path_from_project_root: str = "",
    ...
    """Get ExportBpy single-file Blueprint bpy text."""
    params: Dict[str, Any] = {}
    _apply_blueprint_target_params(
        params,
        target=target,
    ...
    return _forward_unreal_command("get_blueprint_bpy", params)


@mcp.tool()
def export_asset_bpy(
    target: str = "",
    asset_path: str = "",
    blueprint_path: str = "",
    soft_path_from_project_root: str = "",
    ...
    """Export ExportBpy single-file Blueprint bpy using the asset alias."""
    params: Dict[str, Any] = {}
    _apply_blueprint_target_params(
        params,
        target=target,
...
def export_blueprint_bpy(
    ...
    """Export ExportBpy single-file Blueprint bpy."""
    params: Dict[str, Any] = {}
    _apply_blueprint_target_params(
        params,
        target=target,
    ...
    _maybe_add_param(params, "output_file", output_file)
    _maybe_add_param(params, "path", path)
    return _forward_unreal_command("export_blueprint_bpy", params)


def _build_import_blueprint_py_params(
    ...
    _maybe_add_param(params, "spec_json", spec_json)
    _maybe_add_param(params, "python_text", python_text)
    _maybe_add_param(params, "script_text", script_text)
    _maybe_add_param(params, "input_path", input_path)
    _maybe_add_param(params, "script_path", script_path)
    _apply_blueprint_target_params(
        params,
        target=target,
        asset_path=asset_path,
...
def _resolve_blueprint_asset_path(
    target: str = "",
    asset_path: str = "",
    blueprint_path: str = "",
    soft_path_from_project_root: str = "",
    ...
    """Resolve the best available Blueprint asset path for post-import inspection."""
    for candidate in (target, asset_path, blueprint_path, soft_path_from_project_root, blueprint):
...
@mcp.tool()
def import_blueprint_from_bpy_verified(
    bpydata: str = "",
    spec: Optional[Dict[str, Any]] = None,
    spec_json: str = "",
    python_text: str = "",
    ...
    This is a non-breaking companion to import_blueprint_from_bpy. It keeps the
    existing import behavior, but adds a second readback step so callers can
    immediately inspect the resulting Blueprint state. Residual/orphaned fields
    are currently tolerated and reported rather than forcibly removed.
    """
    inline_text = script_text or python_text or bpydata
    import_params = _build_import_blueprint_py_params(
        spec=spec,
        spec_json=spec_json,
        python_text=python_text,
        script_text=inline_text,
    ...
    resolved_asset_path = _resolve_blueprint_asset_path(
        target=target,
        asset_path=asset_path,
        blueprint_path=blueprint_path,
        soft_path_from_project_root=soft_path_from_project_root,
    ...
    if resolved_asset_path:
        ...
        response["post_import_meta"] = _forward_unreal_command(
            "get_asset_meta",
            {
...
def edit_blueprint_by_bpy(
    ...
    """Apply ExportBpy DSL content to an existing target Blueprint."""
    inline_text = script_text or python_text or bpydata
    return _forward_unreal_command(
        "edit_blueprint_by_bpy",
        _build_import_blueprint_py_params(
            spec=spec,
            spec_json=spec_json,
            python_text=python_text,
...
def _build_export_blueprint_functions_params(
    target: str = "",
    asset_path: str = "",
    blueprint_path: str = "",
    soft_path_from_project_root: str = "",
...
@mcp.tool()
def export_blueprint_functions(
    target: str = "",
    asset_path: str = "",
    blueprint_path: str = "",
    soft_path_from_project_root: str = "",
    ...
    """Export Blueprint functions into a directory."""
    return _forward_unreal_command(
        "export_blueprint_functions",
        _build_export_blueprint_functions_params(
            target=target,
            asset_path=asset_path,
            blueprint_path=blueprint_path,
            soft_path_from_project_root=soft_path_from_project_root,
...
def split_blueprint_functions(
    ...
    """Alias for exporting Blueprint functions into a directory."""
    return _forward_unreal_command(
        "split_blueprint_functions",
        _build_export_blueprint_functions_params(
            target=target,
...
def _build_import_blueprint_functions_params(
    input_directory: str = "",
    directory: str = "",
    target: str = "",
    asset_path: str = "",
    ...
    params: Dict[str, Any] = {}
    _maybe_add_param(params, "input_directory", input_directory)
    _maybe_add_param(params, "directory", directory)
    _apply_blueprint_target_params(
        params,
        target=target,
...
@mcp.tool()
def import_blueprint_functions(
    input_directory: str = "",
    directory: str = "",
    target: str = "",
    asset_path: str = "",
    ...
    """Import Blueprint functions from a directory."""
    return _forward_unreal_command(
        "import_blueprint_functions",
        _build_import_blueprint_functions_params(
...
@mcp.tool()
def reimport_blueprint_functions(
    input_directory: str = "",
    directory: str = "",
    target: str = "",
    asset_path: str = "",
    ...
    """Alias for importing Blueprint functions from a directory."""
    return _forward_unreal_command(
        "reimport_blueprint_functions",
        _build_import_blueprint_functions_params(
            input_directory=input_directory,
...
def _build_export_blueprint_target_params(
    target: str = "",
    asset_path: str = "",
    blueprint_path: str = "",
    soft_path_from_project_root: str = "",
...
def export_blueprint(
    ...
    """Export Blueprint text via ExportBlueprintToTxt."""
    return _forward_unreal_command(
        "export_blueprint",
        _build_export_blueprint_target_params(
            target=target,
            asset_path=asset_path,
            blueprint_path=blueprint_path,
            soft_path_from_project_root=soft_path_from_project_root,
...
def exportblueprint(
    ...
    """Alias for Blueprint text export."""
    return _forward_unreal_command(
        "exportblueprint",
        _build_export_blueprint_target_params(
            target=target,
...
def compile_live_coding(
    ...
    Args:
        ...
        show_console: Show the Live Coding console
        enable_if_needed: Enable Live Coding for the current session if needed
    """
    unreal = get_unreal_connection()
    if not unreal:
        return {"success": False, "message": "Failed to connect to Unreal Engine"}
    ...
    try:
        ...
        response = unreal.send_command("compile_live_coding", params)
        return response or {"success": False, "message": "No response from Unreal"}
    except Exception as e:
        logger.error(f"compile_live_coding error: {e}")
        return {"success": False, "message": str(e)}
...
def compile_cpp(
    ...
    """
    Alias for Live Coding compilation of Unreal C++ code.
    ...
    try:
        ...
        response = unreal.send_command("compile_cpp", params)
        return response or {"success": False, "message": "No response from Unreal"}
    except Exception as e:
        logger.error(f"compile_cpp error: {e}")
        return {"success": False, "message": str(e)}

@mcp.tool()
def compile_unreal_code(
    wait: bool = True,
    show_console: bool = True,
    enable_if_needed: bool = True
) -> Dict[str, Any]:
    """
    Alias for Live Coding compilation of Unreal code.
    ...
    try:
        ...
        response = unreal.send_command("compile_unreal_code", params)
        return response or {"success": False, "message": "No response from Unreal"}
    except Exception as e:
        logger.error(f"compile_unreal_code error: {e}")
        return {"success": False, "message": str(e)}

@mcp.tool()
def trigger_live_coding(
    explanation: str = "",
    ...
    """
    Trigger Live Coding compilation or toggle Live Coding on/off.

    If toggle is omitted, this behaves like the Unreal-side trigger entrypoint:
    it compiles and waits by default. If toggle is provided, it enables or
    disables Live Coding for the current session and by default.

    Args:
        explanation: Optional context string returned back by the Unreal command
        toggle: Optional boolean to explicitly enable or disable Live Coding
        wait: Wait for compilation to complete when triggering a compile
        show_console: Show the Live Coding console when triggering a compile
        enable_if_needed: Enable Live Coding for the current session if needed
    """
    unreal = get_unreal_connection()
    if not unreal:
    ...
    try:
        ...
        if toggle is None:
            params["wait"] = wait
            params["show_console"] = show_console
            params["enable_if_needed"] = enable_if_needed
        else:
            params["toggle"] = toggle

        response = unreal.send_command("trigger_live_coding", params)
        return response or {"success": False, "message": "No response from Unreal"}
    except Exception as e:
        logger.error(f"trigger_live_coding error: {e}")
        return {"success": False, "message": str(e)}

@mcp.tool()
def get_live_coding_log(max_lines: int = 200) -> Dict[str, Any]:
    """
    Get the latest Live Coding compile log block from the project's Saved/Logs files.

    Args:
        max_lines: Maximum number of log lines to return from the latest compile block
    """
    params = {"max_lines": max_lines}
    return _forward_unreal_command("get_live_coding_log", params)


@mcp.tool()
def set_gamemode_default_spawn_class(
    spawn_class: str,
    game_mode_blueprint: str = "",
    game_mode_blueprint_path: str = "",
    blueprint_path: str = "",
    blueprint_name: str = "",
    game_mode: str = ""
) -> Dict[str, Any]:
    """Set the default spawn/pawn class on a GameMode Blueprint."""
    params: Dict[str, Any] = {"spawn_class": spawn_class}
    _maybe_add_param(params, "game_mode_blueprint", game_mode_blueprint)
    _maybe_add_param(params, "game_mode_blueprint_path", game_mode_blueprint_path)
    _maybe_add_param(params, "blueprint_path", blueprint_path)
    _maybe_add_param(params, "blueprint_name", blueprint_name)
    _maybe_add_param(params, "game_mode", game_mode)
    return _forward_unreal_command("set_gamemode_default_spawn_class", params)


@mcp.tool()
def set_gamemode_default_pawn_class(
    pawn_class: str,
    game_mode_blueprint: str = "",
    game_mode_blueprint_path: str = "",
    blueprint_path: str = "",
    ...
    """Alias for setting the default pawn class on a GameMode Blueprint."""
    params: Dict[str, Any] = {"pawn_class": pawn_class}
    _maybe_add_param(params, "game_mode_blueprint", game_mode_blueprint)
    _maybe_add_param(params, "game_mode_blueprint_path", game_mode_blueprint_path)
    _maybe_add_param(params, "blueprint_path", blueprint_path)
    _maybe_add_param(params, "blueprint_name", blueprint_name)
    _maybe_add_param(params, "game_mode", game_mode)
    return _forward_unreal_command("set_gamemode_default_pawn_class", params)
...
@mcp.tool()
def read_blueprint_content_fast(
    blueprint_path: str,
    include_event_graph: bool = True,
    include_functions: bool = True,
    include_variables: bool = True,
    ...
    """Read a faster lightweight Blueprint summary."""
    params = {
        "blueprint_path": blueprint_path,
        "include_event_graph": include_event_graph,
        "include_functions": include_functions,
    ...
    return _forward_unreal_command("read_blueprint_content_fast", params)
...
@mcp.tool()
def analyze_blueprint_graph_fast(
    blueprint_path: str,
    graph_name: str = "EventGraph",
    include_node_details: bool = True,
    include_pin_connections: bool = True,
    ...
    """Analyze a Blueprint graph using the bridge's fast path."""
    params = {
        "blueprint_path": blueprint_path,
        "graph_name": graph_name,
        "include_node_details": include_node_details,
    ...
    return _forward_unreal_command("analyze_blueprint_graph_fast", params)
...
@mcp.tool()
def get_blueprint_material_info(blueprint_name: str) -> Dict[str, Any]:
    """Get information about materials currently applied inside a Blueprint."""
    unreal = get_unreal_connection()
    if not unreal:
        return {"success": False, "message": "Failed to connect to Unreal Engine"}
...
def format_graph(
    ...
    Returns:
        Dictionary with success status, node_count, graph_name or error
    """
    unreal = get_unreal_connection()
    if not unreal:
        return {"success": False, "message": "Failed to connect to Unreal Engine"}
    ...
    try:
        result = graph_formatter.format_graph(
            unreal,
            blueprint_name,
            graph_name,
            function_name,
